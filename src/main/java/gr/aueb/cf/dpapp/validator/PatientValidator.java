...
/**
 * Utility class for validating Patient-related DTOs.
 *
...
public class PatientValidator {
    ...
    /**
     * Validates a Patient DTO (or any DTO extending BaseDTO).
    ...
    public static <T extends BaseDTO> Map<String, String> validate(T dto) {
        ...
        if (dto.getFirstname().matches("^.*\\s+.*$")) {
            errors.put("firstname", "Firstname should not include whitespace.");
        }

        // Check for whitespaces in last name
        if (dto.getLastname().matches("^.*\\s+.*$")) {
            errors.put("lastname", "Lastname should not include whitespace.");
        }
